...
describe('pi webservice thresholds', function () {
  it('get all topology thresholds', async function () {
    ...
    const topologyNode1 = res.topologyNodes.find(
      (item) => item.id === 'viewer_rivers_level_stations',
    )
    expect(topologyNode1?.filterLocationsCount).toEqual(4)

    const topologyNode2 = res.topologyNodes.find(
      (item) => item.id === 'viewer_rivers_critical_points',
    )
