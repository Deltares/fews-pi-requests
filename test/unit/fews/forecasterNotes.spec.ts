...
describe('forecasterNotes', function () {
  it('generates a valid forecasterNotes request', async function () {
    const baseUrl =
    ...
    expect(results.noteGroups?.[0].note?.eventCodeId).toBe(
      'Manual.event.notegroup1',
    )
    ...
    expect(results.noteGroups?.[0].note?.messageTemplate?.message).toBe(
      'custom text',
    )
    expect(results.noteGroups?.[0].note?.messageTemplate?.messageHeight).toBe(
      200,
    )
    expect(results.noteGroups?.[0].note?.messageTemplate?.messageWidth).toBe(
      400,
    )
  })
})
