...
describe('/displaygroups/nodes', function () {
  it('test display groups nodes', async function () {
    fetchMock.get(
      'https://mock.dev/fewswebservices/rest/fewspiservice/v1/displaygroups/nodes',
      {
        status: 200,
    ...
    if (
      response.displayGroups.length == 5 &&
      response.displayGroups[4].plotNodes?.length == 2
    ) {
      expect(response.displayGroups[4].plotNodes[0].displayId).toBe('0')
      expect(response.displayGroups[4].plotNodes[0].displayName).toBe(
        'test name',
      )
    }
  })
