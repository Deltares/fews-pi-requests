...
describe('componentSettings', function () {
  it('gets called when done', async () => {
    ...
    expect(results.webOCComponentSettings?.map?.locationsLayer?.show).toBe(true)
    expect(
    ...
    expect(
      results.webOCComponentSettings?.map?.locationsLayer
        ?.locationSearchEnabled,
    ).toBe(true)
    expect(results.webOCComponentSettings?.map?.overlays?.length).toBe(2)
    expect(results.webOCComponentSettings?.map?.overlays?.[0].id).toBe(
      'control_subcatchments',
    )
    expect(results.webOCComponentSettings?.map?.overlays?.[0].visible).toBe(
      true,
    )
    expect(results.webOCComponentSettings?.map?.overlays?.[0].type).toBe(
      'overLay',
    )
    expect(results.webOCComponentSettings?.map?.overlays?.[1].id).toBe(
      'control_branches',
    )
    expect(results.webOCComponentSettings?.map?.overlays?.[1].visible).toBe(
      true,
    )
    expect(results.webOCComponentSettings?.map?.overlays?.[1].type).toBe(
      'gridLayer',
    )
    expect(results.webOCComponentSettings?.charts?.general?.startPanel).toBe(
      'metaDataPanel',
    )
    expect(results.webOCComponentSettings?.charts?.general?.toolBar).toBe(
      'false',
    )
    expect(results.webOCComponentSettings?.charts?.general?.locationNames).toBe(
      false,
    )
    expect(
    ...
    expect(results.webOCComponentSettings?.charts?.actions?.downloadData).toBe(
      true,
    )
    expect(
